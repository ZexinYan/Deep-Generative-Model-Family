...
class EnergyModel(nn.Module):
    def __init__(self, hidden_features=32, out_dim=1, **kwargs) -> None:
        super().__init__()
        ...
        self.cnn_layers = nn.Sequential(
            ...
            nn.Conv2d(c_hid1, c_hid2, kernel_size=3,
                      stride=2, padding=1),  # [8x8]
            Swish(),
            nn.Conv2d(c_hid2, c_hid3, kernel_size=3,
                      stride=2, padding=1),  # [4x4]
            Swish(),
            nn.Conv2d(c_hid3, c_hid3, kernel_size=3,
                      stride=2, padding=1),  # [2x2]
            Swish(),
            nn.Flatten(),
...
class SamplingBuffer:
    def __init__(self, model, img_shape, sample_size, max_len=8192):
        ...
        self.examples = [(torch.rand((1, ) + self.img_shape, device=device) * 2 - 1)
    ...
    def sample_new_examples(self, steps=60, step_size=10):
        ...
        """
        # select 95% from buffer, 5% from scratch
        n_new = np.random.binomial(self.sample_size, 0.05)
        rand_imgs = torch.rand(
            (n_new, ) + self.img_shape, device=device) * 2 - 1
        old_imgs = torch.cat(
            random.choices(self.examples, k=self.sample_size - n_new), dim=0)
        inp_imgs = torch.cat([rand_imgs, old_imgs], dim=0).detach().to(device)

        # perform MCMC sampling
        inp_imgs = SamplingBuffer.generate_samples(
            self.model, inp_imgs, steps, step_size=step_size)
        self.examples = list(inp_imgs.chunk(
            self.sample_size, dim=0)) + self.examples
    ...
    def generate_samples(model, inp_imgs, steps=60, step_size=10, return_img_per_step=False):
        ...
        noise = torch.rand_like(inp_imgs, device=device, requires_grad=False)
...
class DeepEnergyModel(pl.LightningModule):
    ...
    def __init__(self, img_shape, batch_size, alpha=0.1, lr=1e-4, beta1=0.0, **CNN_args):
        ...
        self.cnn = EnergyModel(**CNN_args)
        self.sampler = SamplingBuffer(
            self.cnn, img_shape=img_shape, sample_size=batch_size)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Obtain samples
        fake_imgs = self.sampler.sample_new_examples(steps=60, step_size=10)
...
class GenerateCallback(pl.Callback):
    ...
    def __init__(self, batch_size=8, vis_steps=8, num_steps=256, every_n_epochs=5):
        ...
        # Number of steps within generation to visualize
    ...
    def generate_imgs(self, pl_module):
        ...
        start_imgs = torch.rand(
            (self.batch_size,) + pl_module.hparams["img_shape"]).to(pl_module.device)
        start_imgs = start_imgs * 2 - 1
        # Tracking gradients for sampling necessary
        torch.set_grad_enabled(True)
        imgs_per_step = SamplingBuffer.generate_samples(
...
class SamplerCallback(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        if trainer.current_epoch % self.every_n_epochs == 0:
            exmp_imgs = torch.cat(random.choices(
                pl_module.sampler.examples, k=self.num_imgs), dim=0)
            grid = torchvision.utils.make_grid(
